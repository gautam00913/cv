...
<head>
    ...
    <title>{{ __('messages.forgot_password_title') }}</title>
...
<body class="bg-primary/70">
    <div class="flex items-center justify-center h-screen">
        ...
        <div class="bg-white relative rounded-2xl p-3 md:px-10 mb-5 md:pb-5 w-11/12 md:w-1/2 lg:w-2/5 py-10">
           <h1 class="font-bold uppercase text-center mb-3 text-2xl">{{ __('messages.forgot_password') }}</h1>
           <form action="{{ route('password.sendLink') }}" method="POST">
                ...
                @endif
               <div class="mb-5">
                    <label for="email" class="text-secondary mb-1 block">{{ __('messages.your_email') }}</label>
