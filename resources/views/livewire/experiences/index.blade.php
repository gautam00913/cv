    ...
    <div class="flex flex-wrap gap-3 items-center justify-between">
        ...
        <div x-data="{showDropdown: false}" class="relative" @click.outside="showDropdown = false">
            ...
            <div x-cloak x-show="showDropdown" class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 ring-1 shadow-lg ring-black/5 focus:outline-hidden">
                <a href="{{ route('showCompanies') }}" wire:navigate class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem" tabindex="-1" id="user-menu-item-0">{{ __('messages.companies') }}</a>
                <a href="{{ route('showPositions') }}" wire:navigate class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem" tabindex="-1" id="user-menu-item-0">{{ __('messages.positions') }}</a>
            </div>
        </div>
    </div>
