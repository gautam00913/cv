...
@if ($cat === 'button')
    <button {{ $attributes->merge(['class' => $class, 'wire:loading.class.disabled'] + $tab) }}>
        <div class="flex items-center justify-center">
            <svg @if($target)wire:target="{{ $target }}"@endif wire:loading class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
